...
export function LookReelCard({ reel, onVisibilityChange, shouldRenderIframe, isActive }: LookReelCardProps) {
  ...
  const isMp4 = reel.youtubeId.endsWith(".mp4") || reel.youtubeId.includes(".mp4") || reel.youtubeId.includes("imagekit.io");
  ...
  useEffect(() => {
    if (!isIframeMounted || isMp4) return;
  ...
  useEffect(() => {
    if (!isMp4 && isIframeMounted && iframeRef.current) {
  ...
  }, [isPlaying, isIframeMounted, iframeLoaded, isMp4]);
  ...
  }, [isMuted, isIframeMounted, iframeLoaded, isMp4]);
  ...
  return (
    <div
      ...
      `}
    >
      {/* 1. Dynamic Video (Buffered/Loaded on Visibility) */}
      {isIframeMounted && (
        isMp4 ? (
          <div 
