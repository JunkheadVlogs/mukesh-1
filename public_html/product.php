 ...
 * File: product.php
 * Path: public_html/product.php
 * 
 * This file handles Option A: Server-Side Injection of OG image / Metadata for bots.
 * For normal users, it serves the full interactive React SPA seamlessly.
 */

// 1. Get the product slug from rewrite rule index
$slug = isset($_GET['slug']) ? trim($_GET['slug']) : '';
$slug = basename($slug); // Basic security sanitize

// 2. Load the product records metadata
$products_file = __DIR__ . '/products-meta.json';
$product = null;

if (!empty($slug) && file_exists($products_file)) {
...
// Build modern social meta-tags specifically optimized for WhatsApp Bot crawls
